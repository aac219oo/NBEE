...
function ColorCustom({
  ...
  const [userColors, setUserColors] = React.useState<TColor[]>(customColors);
  ...
  const computedColors = React.useMemo(() => userColors, [userColors]);

  const commitCustomColor = React.useMemo(
    () =>
      debounce((val: string) => {
        updateCustomColor(val);
        setUserColors((prev) =>
          prev.some((c) => c.value === val)
            ? prev
            : [...prev, { isBrightColor: false, name: "", value: val }],
        );
      }, 250),
    [updateCustomColor],
  );

  // Preview value updates while dragging; commit on change only
